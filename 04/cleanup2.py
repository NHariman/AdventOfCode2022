def get_range(range_one, range_two):
	...
	if not range_two:
		return False
	if (range_one[0] <= range_two[0] <= range_one[1] or
		range_one[0] <= range_two[1] <= range_one[1] or
		range_two[0] <= range_one[0] <= range_two[1] or
		range_two[0] <= range_one[1] <= range_two[1]):
		return True
	else:
		return False
