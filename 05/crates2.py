...
# saves the range of items into pop then deletes it from the from_stack
# inserts all the values in the pop stack to the to_stack
def execute_action(action, depot):
	from_stack = action[1] - 1
	to_stack = action[2] - 1
	pop = depot[from_stack][0:action[0]]
	del depot[from_stack][0:action[0]]
	for i in range(len(pop)):
		depot[to_stack].insert(i, pop[i])
	return depot
